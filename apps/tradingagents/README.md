...
## What it does
TradingAgents coordinates multiple role-based LLM agents (fundamental, sentiment, news, technical researchers; trader; risk/portfolio) to debate and propose a trading decision for a given ticker and date.
...
### One-off batch run
Runs a single decision for a ticker/date and prints the result.

```bash
# Option A: set env in env/.env
# TICKER=NVDA
# DATE=2024-05-10
...
## Notes
- Data vendor defaults follow upstream: yfinance for price/technical, Alpha Vantage for fundamentals & news. You can customize inside the batch command or by creating a small wrapper script.
