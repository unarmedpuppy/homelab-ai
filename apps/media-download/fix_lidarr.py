...
def get_lidarr_api_key():
    """Get Lidarr API key from config file or prompt."""
    ...
    # Prompt user
    print("\nLidarr API key not found. Please provide it:")
    print("You can find it in Lidarr: Settings > General > Security > API Key")
    print("Or run this script with the API key as argument: python fix_lidarr.py YOUR_API_KEY")
...
def add_nzbget_client(api_key):
    ...
    if nzbget_client:
        ...
        url = f"{LIDARR_URL}/downloadClient/{nzbget_client['id']}"
        method = requests.put
...
def add_nzbhydra2_indexer(api_key, nzbhydra2_api_key):
    ...
    nzbhydra2_config = {
        ...
        "fields": [
            ...
            },
            {
                ...
                "name": "categories",
                "label": "Categories",
                "value": [3000, 3010, 3040],  # Music categories: 3000=Music, 3010=MP3, 3040=Lossless
                "type": "select"
            },
            {
                "order": 4,
    ...
    if nzbhydra2_indexer:
        ...
        url = f"{LIDARR_URL}/indexer/{nzbhydra2_indexer['id']}"
        method = requests.put
        print(f"Updating existing NZBHydra2 indexer (ID: {nzbhydra2_indexer['id']})...")
    else:
        # Add new indexer
        url = f"{LIDARR_URL}/indexer"
        method = requests.post
...
def add_jackett_indexer(api_key, jackett_api_key):
    ...
    # Get existing Lidarr indexers
    lidarr_indexers = get_indexers(api_key)
    
    # Find existing Torznab indexers that point to Jackett
    existing_jackett_indexers = []
    for lidarr_idx in lidarr_indexers:
        if lidarr_idx.get("implementation") == "Torznab":
            base_url = None
            api_path = None
            for field in lidarr_idx.get("fields", []):
                if field.get("name") == "baseUrl":
                    base_url = field.get("value", "")
            ...
            if base_url and JACKETT_HOST in base_url:
                existing_jackett_indexers.append(lidarr_idx)
    ...
    if not jackett_indexers:
        if existing_jackett_indexers:
            print(f"Found {len(existing_jackett_indexers)} existing Jackett indexers in Lidarr")
            print("⚠ Cannot fetch from Jackett API - please manually configure indexers in Lidarr UI")
            ...
            print("   Please add indexers manually in Lidarr UI")
            return None
    ...
    for jackett_idx in jackett_indexers:
        ...
        # Check if this indexer already exists in Lidarr
        ...
        if existing_indexer:
            ...
            url = f"{LIDARR_URL}/indexer/{existing_indexer['id']}"
            method = requests.put
...
def main():
    print("=" * 60)
    print("Lidarr Configuration Fix")
    print("=" * 60)
    
    # Get Lidarr API key
    print("\n[1/5] Getting Lidarr API key...")
    lidarr_api_key = get_lidarr_api_key()
    if not lidarr_api_key:
        print("✗ Lidarr API key required")
        sys.exit(1)
    print("✓ Lidarr API key obtained")
    
    # Configure NZBGet download client
    print("\n[2/5] Configuring NZBGet download client...")
    nzbget_result = add_nzbget_client(lidarr_api_key)
    if nzbget_result:
        # Test connection
        print("\n[3/5] Testing NZBGet connection...")
        test_nzbget_connection(lidarr_api_key)
    
    # Configure indexers
    print("\n[4/5] Configuring indexers...")
    nzbhydra2_api_key = get_nzbhydra2_api_key()
    add_nzbhydra2_indexer(lidarr_api_key, nzbhydra2_api_key)
    
    jackett_api_key = get_jackett_api_key()
    add_jackett_indexer(lidarr_api_key, jackett_api_key)
    
    print("\n[5/5] Configuration complete!")
    print("\nNext steps:")
    print("1. Verify download clients in Lidarr: Settings > Download Clients")
    print("2. Verify indexers in Lidarr: Settings > Indexers")
    print("3. Test a search in Lidarr to verify indexers are working")
    print("4. If issues persist, check Lidarr logs")
