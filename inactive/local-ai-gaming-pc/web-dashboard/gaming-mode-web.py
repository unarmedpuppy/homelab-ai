...
Runs in Docker container, accessible at http://localhost:8080
"""
...
app = Flask(__name__)
# Use manager service name when in Docker, localhost when running standalone
MANAGER_URL = os.getenv("MANAGER_URL", "http://vllm-manager:8000")
# For health checks, try localhost if manager service name doesn't work
MANAGER_URL_FALLBACK = os.getenv("MANAGER_URL_FALLBACK", "http://localhost:8000")
    ...
    <script>
        ...
        function updateStatus(data) {
            if (!data) {
                ...
                showError('Cannot connect to Local AI Manager. Make sure it\'s running.');
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    if (result.failed && result.failed.length > 0) {
                        showError(`Stopped ${result.stopped.length} model(s), but ${result.failed.length} failed.`, 'error');
                    } else {
...
def get_manager_url():
    """Try manager service name first, fallback to localhost."""
    try:
        response = requests.get(f"{MANAGER_URL}/healthz", timeout=2)
    ...
    try:
        response = requests.get(f"{MANAGER_URL_FALLBACK}/healthz", timeout=2)
        if response.status_code == 200:
            return MANAGER_URL_FALLBACK
    except:
        pass
...
if __name__ == '__main__':
    ...
    print(f"Starting Gaming Mode Web Dashboard on {host}:{port}")
