...
class SQLiteMemory:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        Args:
            db_path: Path to SQLite database (defaults to agents/memory/memory.db)
        """
        if db_path is None:
    ...
    def query_decisions(
        ...
        if search_text:
            ...
            joins.append("JOIN decisions_fts fts ON d.id = fts.rowid")
        ...
        if tags:
            joins.append("JOIN decision_tags dt ON d.id = dt.decision_id")
            joins.append("JOIN tags t ON dt.tag_id = t.id")
    ...
    def query_patterns(
        ...
        if search_text:
            # Use full-text search
            joins.append("JOIN patterns_fts fts ON p.id = fts.rowid")
        ...
        if tags:
            joins.append("JOIN pattern_tags pt ON p.id = pt.pattern_id")
    ...
    def export_to_markdown(self, output_path: Optional[Path] = None) -> Path:
        ...
        Args:
            output_path: Directory to export to (defaults to agents/memory/memory/)
